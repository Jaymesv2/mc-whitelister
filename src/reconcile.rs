...
use luckperms_api::apis::{groups_api, users_api};
...
pub async fn reconcile_luckperms(state: &Arc<AppState>) -> Result<(), ReconcileErrors> {
    ...
    for agroup in &agroups {
        if lgroups.iter().find(|x| **x == agroup.data.name).is_none() {
            info!(
                "authentik group {} did not exist in luckperms, creating minecraft group named {}",
                agroup.name, agroup.data.name
            );
            groups_api::create_group(
                &state.luckperms,
                Some(luckperms_api::models::new_group::NewGroup {
    ...
    for i in accounts {
        if let Ok(iuuid) = Uuid::parse_str(&i.uuid) {
            authentik_uid_uuid_mapping
                .entry(i.user_id)
                .or_insert(vec![])
                .push(ResponseAccount {
                    uuid: iuuid,
                    username: i.username,
                });
        } else {
    ...
    for (auid, account) in authentik_uid_uuid_mapping
        .iter()
        .flat_map(|(uid, v)| v.iter().map(move |x| (uid, x)))
        .collect::<Vec<_>>()
    {
        if luckperms_users
            .iter()
            .find(|uuid| account.uuid == **uuid)
            .is_none()
        {
            info!(
                "creating user \"{}\" with uuid \"{}\" in luckperms",
                account.username, account.uuid
            );
            users_api::create_user(
                &state.luckperms,
                Some(luckperms_api::models::new_user::NewUser::new(
                    account.uuid,
        ...
        info!(
            "expecting {} to have groups: {:?}",
            account.username, expected_groups
        );

        let to_remove: Vec<&String> = user_groups
            .iter()
            .filter(|gname| {
                luckperms_group_names.contains(gname)
                    && expected_groups.iter().find(|n| *n == gname).is_none()
            })
            .collect();

        if !to_remove.is_empty() {
            info!("removing groups {:?} from {}", to_remove, account.username);
            users_api::clear_user_nodes(
                &state.luckperms,
                &user_uuid,
                Some(
                    to_remove
                        .into_iter()
                        .map(|name| {
                            luckperms_api::models::new_node::NewNode::new(format!("group.{}", name))
                        })
                        .collect(),
                ),
            )
        ...
        let to_add: Vec<&String> = expected_groups
            .iter()
            .filter(|gname| {
                luckperms_group_names.contains(*gname)
                    && user_groups.iter().find(|n| n == *gname).is_none()
            })
            .copied()
            .collect();
        ...
        if !to_add.is_empty() {
            ...
            users_api::add_user_nodes(
                ...
                Some(
                    to_add
                        .into_iter()
                        .map(|name| luckperms_api::models::new_node::NewNode {
...
async fn get_authentik_groups(
    state: &Arc<AppState>,
) -> Result<Vec<AuthentikGroup>, authentik_client::apis::Error<CoreGroupsListError>> {
    let authentik_groups_req = core_api::core_groups_list(
        &state.authentik,
    ...
    Ok(authentik_groups_req
        ...
        .filter_map(|group| {
            ...
            let luckperms_data = match serde_json::from_value::<AuthentikLuckpermsGroupAttribute>(
                ...
                Err(e) => {
                    warn!(
                        "invalid luckperms attribute on group {}, \"{:?}\"",
                        group.name, e
                    );
                    return None;
                }
    ...
    fn into(self) -> Vec<luckperms_api::models::new_node::NewNode> {
        self.nodes
            ...
            .map(|x| luckperms_api::models::new_node::NewNode {
                key: x.key,
                value: Some(x.value),
                context: x.context.map(|u| {
                    u.into_iter()
                        .map(|y| luckperms_api::models::context::Context {
                            key: y.key,
                            value: y.value,
                        })
                        .collect()
